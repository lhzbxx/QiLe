...
def pay_success():
	...
	room = query_db('select merchant_uuid, room_name, stock from rooms where uuid = ?', [order['room_uuid']], one=True)
	...
	g.db.execute('update orders set deal_state = 1 where uuid = ?', [p, t1])
